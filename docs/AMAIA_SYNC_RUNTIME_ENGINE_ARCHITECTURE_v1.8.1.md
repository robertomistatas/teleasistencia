# AMAIA-SYNC Runtime Engine Architecture v1.8.1

**Type:** Runtime architecture blueprint  
**Status:** Pending Codex hostile re-audit  
**Supersedes:** v1.8 (micro consistency revision — no new concepts)  
...
| # | Type | Fix |
|---|---|---|
| A | Consistency | Section 8.5: removed obsolete initial failure table that contradicted the final simplified model. Single rollback model remains. |
| B | Consistency | Sections 7.1, 8.4: removed all PostgreSQL latency assumptions ("sub-millisecond", "commit latency"). Architecture does not rely on timing. |
