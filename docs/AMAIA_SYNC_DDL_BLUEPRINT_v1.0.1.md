# AMAIA-SYNC DDL Blueprint v1.0.1

**Phase:** 9.3A  
**Status:** DDL Blueprint — non-executable, pending Codex audit before migration authoring  
**Supersedes:** AMAIA_SYNC_DDL_BLUEPRINT_v1.0.md (removed executable SQL fragments)  
...
**Note:** This document contains NO executable SQL. All DDL is expressed as conceptual specifications. Executable statements will be authored as migration files in a subsequent phase after Codex approval of this blueprint.
...
| current_investigation_id | uuid | NULL | | FK added in Step 4 via deferred constraint operation |
...
All simple FKs are defined inline with their table definitions in Step 1.

### Step 3 — Composite UK prerequisites

The following UKs must exist before their composite FKs can reference them. They are defined inline with their table definitions in Step 1:
...
### Step 4 — Circular composite FK (deferred constraint)

After both subjects and investigations tables exist, add the composite FK constraint on amaia_sync_manifest_exclusion_subjects:

- **Constraint name:** fk_current_investigation
- **Source columns:** (current_investigation_id, id)
- **Target:** amaia_sync_manifest_exclusion_investigations(id, subject_id)
- **ON DELETE:** RESTRICT
- **ON UPDATE:** NO ACTION

This is the sole deferred constraint operation that requires both tables to exist first. All other FKs are defined inline with their table creation. The future migration will implement this as a post-creation constraint addition.
...
Modify amaia_sync_runs: add columns cycle_id, upstream_run_id, blocked_entity_name. Drop and re-create reason_code CHECK constraint with the extended value set. Add indexes on cycle_id and upstream_run_id.

Modify amaia_sync_reconciliation_results: add columns cycle_id, scope_descriptor, result_status. Add result_status CHECK constraint. Add index on cycle_id.

Modify amaia_beneficiaries, amaia_support_network, and amaia_alerts: drop and re-create sync_status CHECK constraint on each with the extended value set including 'reactivation_pending'.

### Step 7 — Seeds and data corrections

Insert 12 reconciliation_segments seed rows (domain_name='alerta', tier='tier4', segment_id 0–11). Correct the amaia_sync_watermarks record for entity_name='alerta' (see Part 6 for details).
...
Correct the amaia_sync_watermarks record where entity_name = 'alerta': set watermark_type to 'id', last_id to 0, last_timestamp to NULL, and watermark_expr to 'derived:logestado.amaia_id→amaia_alert_logs.alert_amaia_id'.
