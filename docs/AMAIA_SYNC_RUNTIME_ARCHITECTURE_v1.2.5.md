# AMAIA-SYNC Runtime Architecture v1.2.5

**Phase:** 9.3 Rev.8  
**Status:** Design — pending internal review  
**Supersedes:** AMAIA_SYNC_RUNTIME_ARCHITECTURE_v1.2.4.md  
...
**New DDL:** 1 new table (amaia_sync_alert_remediation_queue), 2 new columns on existing tables.
...
If raw_max_id has been unchanged across consecutive cycles for at least safety_lag_time (configurable, default 5 minutes), all IDs up to raw_max_id are safe. The reasoning: if no new IDs are being assigned, there are no concurrent transactions that could be holding invisible IDs in the gap zone.
...
When AMAIA is inactive for >= safety_lag_time: safe_by_time = raw_max_id, confirmed_upper_bound = raw_max_id. The entire range is promoted to safe. The tail is processed. No starvation.

### Provisional processing of the lag zone

When confirmed_upper_bound < raw_max_id, the zone (confirmed_upper_bound, raw_max_id] contains IDs that are probably committed but not yet confirmed safe. These are processed provisionally.
...
- **Starvation:** Temporal promotion ensures the tail is always processed within safety_lag_time.
...
- **Correctness:** The confirmed watermark only advances over confirmed-safe IDs. Provisional IDs are validated when they enter the confirmed range.
...
2 new columns on amaia_sync_run_manifests (provisional_upper_bound, provisional_id_count, provisional_id_hash — wait, that's 3). Let me correct: 3 new columns.
...
No starvation. Maximum tail delay: safety_lag_time (5 minutes).
...
| No starvation of safety lag tail | Temporal promotion after safety_lag_time of raw_max_id stability |
...
All v1.2.4 SLOs are preserved. The safety lag tail SLO is upgraded:

| SLO | v1.2.4 | v1.2.5 |
|---|---|---|
| Safety lag tail processing | Depends on raw_max_id growth | Guaranteed within safety_lag_time via temporal promotion |

Note: "guaranteed within safety_lag_time" assumes the engine runs at least one cycle within that window. If the engine itself is down, the tail is delayed until restart. This is an engine-availability dependency, not a safety lag design issue.

### Cannot be guaranteed

All v1.2.4 "cannot be guaranteed" items remain. No new items added.
