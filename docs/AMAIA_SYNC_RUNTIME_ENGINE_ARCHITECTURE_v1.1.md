# AMAIA-SYNC Runtime Engine Architecture v1.1

**Type:** Runtime architecture blueprint  
**Status:** Pending Codex hostile audit  
**Supersedes:** v1.0 (pre-Codex hardening — 7 architectural observations addressed)  
...
| # | Observation | Change |
|---|---|---|
| A1 | Pre-fence upper_bound safety | Added Section 9.1: formal safety proof for pre-fence extraction. |
| A2 | Scheduler heartbeat protocol | Added Section 4.1: scheduler lease heartbeat with renewal points and expiry abort. |
| A3 | Batching policy | Added Section 9.2: incremental batch processing (1000 rows/page), single manifest, single finalization. |
| A4 | Stale-run detection threshold | Added threshold formula in Section 15: `stale = lease expired AND heartbeat > 2*TTL`. |
| A5 | Empty incremental watermark semantics | Hardened Section 9 (Empty incrementals): NO CAS on empty, watermark unchanged, explicit rationale. |
| A6 | Domain-level transaction boundary | Added Section 10.1: one manifest, one CAS attempt per domain per cycle. No partial manifests. |
| A7 | Architectural invariants | Added Section 26: 10 numbered invariants for Codex audit. |
...
The scheduler lease has a fixed TTL (default: 5 minutes). The engine renews it at these points:

1. **Before** starting each domain (pre-domain heartbeat).
2. **After** completing each domain (post-domain heartbeat).
3. **Periodically** during long-running domain processing (every 60 seconds).

Renewal updates `heartbeat_at = now()` and `lease_expires_at = now() + TTL`.

**Expiry abort rule:** Before starting any new domain, the engine MUST verify that the scheduler lease is still valid (`lease_expires_at > now()`). If the lease has expired (heartbeat failed, network partition, etc.), the engine MUST NOT start a new domain. It marks the current cycle as incomplete and exits the cycle loop. The lease expires naturally, and the next startup re-acquires it.

**No new domain may start if scheduler lease validity cannot be confirmed.**

A scheduler lease expiration immediately aborts further domain processing within the current cycle.
...
The upper_bound and AMAIA fetch occur **before** the Supabase fenced transaction. This may appear to introduce a TOCTOU risk: what if the watermark changes between the pre-fence read and the fenced CAS? The answer is that this is safe in V1:

1. **Single serialized orchestrator.** Only one engine process exists. The scheduler lease guarantees global exclusivity. No concurrent process can advance any watermark.
2. **Domain lease exclusivity.** Within a cycle, each domain is processed sequentially. The domain lease prevents any other process from writing to the same domain's tables.
3. **No multi-worker.** There are no parallel workers that could consume the same range concurrently.
4. **CAS revalidation under fence.** Even if the above guarantees were violated, the watermark CAS inside the fenced transaction validates `last_id IS NOT DISTINCT FROM expected_cursor`. If any cursor modification occurred between the pre-fence read and the CAS, the CAS fails and the entire transaction rolls back.
5. **Lease expiry invalidates the commit.** If the engine dies between the pre-fence fetch and the fenced transaction, the scheduler lease and domain lease expire. The watermark is never advanced (CAS never executed). The next cycle re-reads the watermark and reprocesses the same range.

**Pre-fence extraction is safe because watermark advancement remains fully fence-protected by CAS.** The pre-fence read is an optimization (avoids holding the domain lease during AMAIA queries), not a correctness risk.
...
Advancing a watermark without observing new source elements provides no value and increases audit ambiguity. The CAS is only called when `sets_match=true` AND the range contains at least one source element AND the upper_bound exceeds the current watermark.

This is a valid empty incremental per Protocol v1.6.4 M1.

### 9.2 Incremental Batch Processing

The runtime fetches AMAIA rows in pages to limit memory consumption and WAL pressure:

- **Page size:** 1000 rows (configurable via `SYNC_BATCH_SIZE`, default 1000).
- **Fetch loop:** `SELECT * FROM :source WHERE cursor > :page_start AND cursor <= :upper ORDER BY cursor LIMIT :batch_size`.
- **All pages belong to the same run and the same manifest.**
- **Source identity items are inserted progressively** as each page is fetched (all at `phase=created`).
- **Destination upserts are performed progressively** as each page is fetched (inside the fenced transaction).

After all pages are exhausted:

- `finalize_source` is called **once** (computes hash over all source items).
- `finalize_comparison` is called **once** (queries full range from destination).
- CAS is called **once** (if `sets_match=true`).
- `complete_manifest` is called **once**.

**Manifest finalization remains single-transaction and atomic.** The batching affects only the AMAIA fetch and destination upsert loops. There are no per-batch manifests, no per-batch CAS, and no partial finalization.
...
There are no:
- Partial manifests (a manifest either reaches a terminal phase or is abandoned).
- Partial CAS (CAS is a single atomic UPDATE with row_count validation).
- Per-batch CAS (batching affects fetch/upsert loops, not finalization).
- Per-batch manifests (all batches contribute to one manifest).

One domain, one run, one manifest, one transaction, one CAS. This is the fundamental atomicity unit of the sync engine.
...
A run is considered **stale** when BOTH conditions are met:

1. The domain lease credentials do not match the run's credentials, OR the lease has expired (`lease_expires_at <= now()`).
2. The run's last activity is older than the stale threshold.
...
A run is only considered stale when `now() > stale_threshold`. The 2x TTL buffer prevents accidental abandonment of engines that are slow but alive (e.g., processing a large batch with heartbeat jitter).
...
The following invariants are declared for Codex audit. Violation of any invariant constitutes an architectural breach.

| # | Invariant |
|---|---|
| R1 | **Single scheduler.** At most one engine process holds the scheduler lease at any time. The scheduler lease is cooperative serialization, not DB authentication. |
| R2 | **Single worker.** Each domain is processed by exactly one thread in exactly one process. No parallelism within or across domains in V1. |
| R3 | **AMAIA readonly.** The engine never writes to AMAIA. SELECT only. No stored procedures, no temp tables, no session variables. |
| R4 | **Fetch before fence.** AMAIA extraction occurs before the Supabase fenced transaction. The watermark is revalidated under fence by CAS. Pre-fence extraction is safe because watermark advancement is fence-protected. |
| R5 | **Finalization under fence.** All manifest finalization (source hash, comparison, provisional, completion) occurs inside the fenced transaction that holds the domain lease. |
| R6 | **CAS inside same transaction.** Watermark advancement via CAS executes in the same transaction as manifest evidence, phase advancement, and exclusion consumptions. All commit or all rollback. |
| R7 | **No watermark advancement without manifest completion.** The CAS is called only when `sets_match=true` AND the manifest has been successfully finalized. A failed manifest never advances the watermark. |
| R8 | **Recovery cannot abandon healthy runs.** A run whose credentials match the current domain lease AND whose lease has not expired is healthy. Abandon is rejected. |
| R9 | **Empty incrementals never advance watermarks.** When `safe_upper_bound <= lower_bound`, the manifest completes with zero items and `sets_match=true`, but no CAS is executed. The watermark does not move. |
| R10 | **Multi-process deployment unsupported in V1.** A second engine process with shared credentials that bypasses the scheduler lease is an operational misconfiguration. It is not defended against by the database. It is prevented by operational discipline, startup validation, and lease detection. |

---

## Summary

The AMAIA-SYNC Runtime Engine is a single-process, single-threaded Node.js daemon deployed on AMAIASQL. It extracts incremental changes from AMAIA MySQL in batches, persists them into Supabase, and constructs cryptographic identity manifests that prove the fidelity of every sync operation. It uses the lease-based fencing, manifest finalization, and CAS watermark protocol deployed in Phase 9.3C. Pre-fence extraction is safe because watermark advancement is fence-protected by CAS. Each domain produces exactly one manifest per cycle. Empty incrementals never advance watermarks. Failures are isolated per domain, recovered on restart via stale-run detection with 2x TTL threshold, and auditable through structured logs and database records. 10 architectural invariants (R1–R10) are declared for hostile audit.
