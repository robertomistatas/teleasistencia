...
- admin: rol administrativo con capacidad de ver y operar todo el sistema, incluyendo beneficiarios, asignaciones, importaciones, llamadas, seguimientos, tareas, snapshots y reportes, excepto configuración crítica.
...
Excel de asignaciones
  -> upload controlado
  -> assignment_import_runs / assignment_imports
...
- se relaciona en forma uno a uno con auth.users usando el mismo identificador,
- puede estar vinculado a registros creados por usuario, como manual_followups, beneficiary_import_runs, assignment_import_runs, call_import_runs y report_runs,
...
- no existe un auth_user_id separado,
...
- representar conceptualmente las ejecuciones de importación como unidades auditables separadas por dominio.

Relaciones:

- no se implementa como una tabla genérica única,
- se materializa en tres tablas de ejecución separadas: beneficiary_import_runs, assignment_import_runs y call_import_runs,
- cada tabla de ejecución se relaciona con sus tablas detalle del dominio correspondiente,
- las tres se relacionan con profiles como usuario iniciador.

Reglas clave:

- toda carga o proceso de importación debe tener una ejecución de dominio específica,
- el archivo de origen, sus metadatos y el resultado de la importación deben quedar trazados,
- las importaciones no se mezclan en una sola tabla genérica,
...
- beneficiary_import_runs: altas o actualizaciones del padrón de beneficiarios cuando exista ese proceso formal.
- assignment_import_runs: cargas de asignaciones por teleoperadora.
- call_import_runs: importaciones globales de llamadas desde AMAIA o Net2Phone.
...
- muchos a uno con assignment_import_runs,
...
- assignment_imports no crea beneficiarios,
- si una fila corresponde a un RUT no existente, se marca error o no encontrado y no se crea automáticamente un beneficiario.

Campos mínimos conceptuales:

- id,
- assignment_import_run_id,
- row_number,
- rut_normalized,
- imported_name,
- target_profile_id,
...
- muchos a uno con call_import_runs,
...
- el matching base se realiza por phone_normalized contra beneficiary_phones,
- una llamada importada puede quedar sin match, con match único o con match ambiguo,
- si existe exactamente un match por teléfono, el match es válido,
- si existen múltiples matches por teléfono, el resultado es conflicto,
- si no existe match por teléfono, el resultado es unmatched,
- luego del match telefónico se intenta resolver la asignación vigente del beneficiario matched,
...
- el catálogo oficial de tipificación es cerrado: contacto_efectivo, no_responde, ocupado, mensaje_dejado, numero_invalido, rechaza_llamada,
- solo contacto_efectivo cuenta como contacto efectivo,
- la app debe distinguir la tipificación en UI, auditoría y derivaciones operacionales.
...
- notes opcional,
- phone_used opcional,
- is_effective_contact opcional, pero si existe debe derivarse estrictamente desde typification_code y solo puede ser verdadero cuando typification_code = contacto_efectivo,
- created_at,
- updated_at.
...
- incorpora seguimientos manuales desde manual_followups cuando typification_code = contacto_efectivo,
...
2. El sistema crea un assignment_import_run.
...
7. Si el RUT no existe en beneficiaries, la fila se marca como error o no encontrado y assignment_imports no crea un beneficiario nuevo.
8. Si el mismo beneficiario aparece en dos cargas que implican dos asignaciones activas incompatibles, se marca conflicto obligatorio.
9. Solo cuando la fila es válida y no conflictiva se crea o actualiza beneficiary_assignments.
10. La vista v_current_assignments expone el resultado operacional vigente.
11. El assignment_import_run queda cerrado con métricas de aceptadas, rechazadas y conflictivas.
...
7. Si la tipificación es contacto_efectivo, las vistas derivadas de contacto efectivo y cobertura se actualizan en consecuencia.
...
2. El sistema crea un call_import_run.
3. Cada fila se persiste primero en calls_raw con payload crudo y campos relevantes extraídos.
4. Un proceso de reconciliación busca match por phone_normalized contra beneficiary_phones.
5. Si existe un único match, el resultado es válido; si existen múltiples matches, el resultado es conflicto; si no existe ninguno, el resultado es unmatched.
6. Luego del match telefónico se intenta resolver la asignación vigente del beneficiario identificado.
7. El resultado se registra en call_matches con estado de match y razón.
8. Se determina si la llamada constituye contacto efectivo conforme a la regla:
   - duración mayor a 10 segundos, o
   - resultado explícito exitoso.
9. Las llamadas con match válido y efectividad positiva alimentan v_effective_contacts.
10. Las vistas de última fecha efectiva y cobertura se recalculan al consultar o mediante materialización, si luego se define.
11. El call_import_run queda auditado con totales, advertencias y errores.
...
- assignment_imports no crea beneficiarios.
- Si el RUT del archivo no existe en beneficiaries, la fila se marca como error o no encontrado.
- Un beneficiario solo puede tener una asignación activa.
...
- Debe incluir tipificación obligatoria.
- El catálogo oficial es cerrado: contacto_efectivo, no_responde, ocupado, mensaje_dejado, numero_invalido y rechaza_llamada.
- Solo contacto_efectivo cuenta como contacto efectivo.
...
- calls_raw conserva la evidencia importada y call_matches conserva su interpretación canónica.
...
- todos los beneficiarios y su operación,
- todos los teléfonos,
- todas las asignaciones,
- todas las importaciones y sus errores,
- todas las llamadas crudas y conciliadas,
...
- todas las tareas,
- todos los snapshots y reportes,
- toda la operación diaria del sistema, excepto configuración crítica.
...
- manual_followups: teleoperadora puede leer seguimientos de beneficiarios asignados a su cartera y crear seguimientos sobre su cartera; admin y super_admin leen todos.
- beneficiary_tasks: acceso análogo al beneficiario asociado.
- beneficiary_import_runs, assignment_import_runs, call_import_runs, assignment_imports, calls_raw y call_matches: restringidos a roles administrativos salvo que se habilite una vista resumida para teleoperadoras.
...
Las unidades principales de auditoría del proceso son beneficiary_import_runs, assignment_import_runs y call_import_runs, separadas por dominio.
...
- usar beneficiary_import_runs, assignment_import_runs, call_import_runs y tablas detalle,
