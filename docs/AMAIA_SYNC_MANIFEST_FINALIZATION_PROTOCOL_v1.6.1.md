# AMAIA-SYNC Manifest Finalization Protocol v1.6.1

**Type:** Subsystem protocol blueprint  
**Status:** Pending Codex final audit  
**Supersedes:** v1.6 (documental hardening only — zero architectural changes)  
**Parent patch:** Schema Patch v1.6  
**Deployed baseline:** Commit dc7574c  
**Author:** Claude (constructor)  
**Date:** 2026-06-22

**Note:** NO SQL, NO migrations, NO runtime code. No new tables, columns, functions, triggers, constraints, or roles. Wording precision only.

---

## Executive Summary

v1.6 delivered the hardened protocol. v1.6.1 applies final documental precision: closes the index inventory placeholder, strengthens the UPDATE privilege trust boundary declaration, and adds explicit self-audit for the operational trust boundary. Zero architectural changes. Zero new DDL objects.
...
| # | Area | Change type |
|---|---|---|
| 1 | Index inventory | Placeholder removed. Exactly 12 indexes listed. No reserved slots. |
| 2 | UPDATE privilege wording | Declared as operational trust boundary, not schema enforcement. |
| 3 | Privilege surface summary | Explicit: lock capability via UPDATE, constrained by operational discipline. |
| 4 | Residual risks | Added: manifest_owner UPDATE privilege operational trust boundary. |
| 5 | Self-audit | Added: manifest_owner function issues UPDATE against lease scenario. |
| 6 | Global cleanup | Removed all instances of: reserved, placeholder, if needed, may need reverify. |
