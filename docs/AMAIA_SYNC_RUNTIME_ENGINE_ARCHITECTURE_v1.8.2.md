# AMAIA-SYNC Runtime Engine Architecture v1.8.2

**Type:** Runtime architecture blueprint  
**Status:** Pending Codex hostile re-audit  
**Supersedes:** v1.8.1 (final consistency micro-fix — no new concepts)  
...
| # | Type | Fix |
|---|---|---|
| C | Consistency | Section 4.3: removed contradictory "failure after bind → bound credentials" wording. Aligned with Section 8.5, Section 15.1, and R35: all post-rollback runs are unbound. |
...
**Post-rollback (any other fenced tx failure):** Pre-run returns to its original NULL-credential state — bind-run-to-lease occurs inside the fenced tx and is reverted by rollback (R35). Recovered by the unbound stale predicate at next cycle start (R28, R34).
...
The architecture does NOT guarantee the lease is still valid at the physical COMMIT timestamp — COMMIT itself is not interceptable. The architecture guarantees exclusively that the lease predicate evaluated TRUE immediately before terminal writes within the same transaction. No timing assumptions are made.
...
The architecture guarantees exclusively that **the lease predicate evaluated TRUE immediately before terminal writes within the same transaction.** COMMIT-time lease validity is not claimed. No timing assumptions are made.
...
### 8.5 Post-Rollback Run State

Since bind-run-to-lease occurs inside the fenced transaction, all fenced transaction rollbacks restore the pre-run to its original NULL-credential state.

| Failure point | Run state after rollback | Credentials | Recovery path |
|---|---|---|---|
| AMAIA fetch failure | No run exists | — | N/A (R29) |
| Any fenced tx failure (steps 2–11) | Unbound, `running` | NULL | Unbound stale (R34) or `skipped_lock_contention` (R30) |

All fenced tx changes roll back atomically, including bind. No alternative interpretation exists.
...
The AMAIA-SYNC Runtime Engine v1.8.2 is a single-process, single-threaded Node.js daemon on AMAIASQL synchronizing 2 append-only id-based domains. Pre-runs carry NULL credentials and are created only after successful AMAIA fetch. The fenced transaction binds the run to the domain lease, hard-revalidates the watermark, produces all evidence, and terminates via **Guarded Terminalization** — a single operation that evaluates the lease predicate TRUE immediately before terminal writes within the same transaction, then completes the manifest and sets the terminal run status. COMMIT-time lease validity is not claimed. No timing assumptions are made. All post-rollback runs are unbound (bind reverts with the tx). Unbound recovery uses exclusively run-local state (R34); bound recovery uses domain lease predicates. Recovery runs at startup and before every cycle. 35 invariants (R1–R35).
