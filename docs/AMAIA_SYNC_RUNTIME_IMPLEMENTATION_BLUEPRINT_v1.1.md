# AMAIA-SYNC Runtime Implementation Blueprint v1.1

**Type:** Implementation blueprint  
**Status:** Pending hostile implementation audit  
**Supersedes:** v1.0 (3 implementation clarifications — no architectural changes)  
...
| # | Area | Clarification |
|---|---|---|
| B1 | Empty incremental flow | Added Section 8.1: dedicated 15-step empty path. Runs and manifests always created. |
| B2 | Guarded Terminalization time source | SQL-time only. JS clock, cached leases, tx-start timestamps prohibited. Added to Section 11 + invariant mapping. |
| B3 | PostgreSQL access strategy | Added Section 13.1: direct `pg` driver connection. supabase-js insufficient for V1 tx requirements. |
...
│   ├── repositories/
│   │   ├── pg-client.ts             # Direct pg driver: connection pool + BEGIN/COMMIT/ROLLBACK + SET LOCAL ROLE
...
async executeDomain(cycle, domain):
  ...
  if (bounds.isEmpty) return await executeEmptyIncremental(cycle, domain, watermark)  // Section 8.1
  rows = await amaiaFetcher.fetch(domain, bounds)     // R4, R29
  preRun = await runService.createPreRun(cycle, domain, bounds)  // R22

  // FENCED TRANSACTION
  try {
    await pg.query('BEGIN')                                               // step 1
    await pg.query('SET LOCAL ROLE amaia_sync_runtime')
...
When `effective_upper_bound <= lower_bound`, no eligible rows exist in the current safe bounded window. **Empty incrementals are still fully audited runtime executions.** They create runs and manifests. They never advance watermarks.

```
async executeEmptyIncremental(cycle, domain, watermark):
  // PRE-FENCE
  bounds = { lower: watermark.last_id, upper: watermark.last_id, isEmpty: true }
  // No AMAIA fetch (nothing to fetch).
  preRun = await runService.createPreRun(cycle, domain, bounds)  // R22, R29 satisfied: fetch not attempted, not failed

  // FENCED TRANSACTION (same 12-step structure)
  try {
    await pg.query('BEGIN')                                       // step 1
    await pg.query('SET LOCAL ROLE amaia_sync_runtime')
    lease = await leaseManager.acquireDomainLease(domain)         // step 2
    await runService.bindToLease(preRun, lease)                   // step 3
    await watermarkService.hardRevalidate(domain, bounds)         // step 4, R24
    manifest = await manifestService.createManifest(preRun, domain) // step 5
    // step 6: zero source items inserted (none fetched)
    // step 7: zero destination upserts (none fetched)
    await manifestService.finalizeSource(manifest, preRun)        // step 8: count=0, hash=hash("")
    setsMatch = await manifestService.finalizeComparison(manifest, preRun) // step 9: sets_match=true
    // step 10: CAS SKIPPED (source_id_count=0, R9)
    await manifestService.guardedTerminalization(manifest, preRun, lease, true) // step 11: status='success'
    await pg.query('COMMIT')                                      // step 12
  } catch (e) {
    await pg.query('ROLLBACK')
    if (isLockContention(e))
  ...
  // POST-COMMIT
  await leaseManager.releaseDomainLease(domain, lease)
```

**Empty incrementals create runs and manifests.** The manifest records `source_id_count=0`, `sets_match=true`, `phase=comparison_complete`. The run records `status='success'`. **Empty incrementals never advance watermarks** (R9).
...
Guarded Terminalization must evaluate the lease predicate **inside PostgreSQL** using SQL-time (`now()`). The following are explicitly prohibited as authoritative time sources:

- JavaScript `Date.now()` or any wall clock.
- Cached lease objects read earlier in the transaction.
- Transaction-start timestamps.

The implementation must issue a single SQL statement (or tightly-coupled sequence within one `pg.query` batch) that:

1. Reads `lease_expires_at` from `amaia_sync_leases` WHERE `entity_name=:domain`.
2. Asserts `lease.owner_identity = run.owner_identity AND lease.lease_token = run.lease_token AND lease.lease_expires_at > now()`.
3. On success: calls `amaia_sync_complete_manifest(manifest_id, run_id)`.
4. Updates `amaia_sync_runs SET status=:terminal_status, finished_at=now()`.
5. On predicate failure: raises exception → transaction rollback.

**No runtime-side lease validity checks are authoritative.** Only the SQL-evaluated predicate inside the fenced transaction is the source of truth for R27/R31/R32.
...
Runtime V1 requires multi-statement transactions with `SET LOCAL ROLE`, `SELECT ... FOR UPDATE`, and transaction-scoped rollback. **`supabase-js` alone is insufficient for these requirements** — it does not expose raw transaction control.

**Primary mechanism:** Node.js `pg` driver connected directly to the Supabase PostgreSQL endpoint (session pooler, not transaction pooler).

```
Driver: pg (node-postgres)
Connection string: postgresql://postgres.[project-ref]:[service-role-password]@[host]:5432/postgres
Pool size: 1 (single-threaded engine)
SSL: required
```

**Responsibilities of `pg-client.ts`:**

- Manage a single `pg.Pool` with `max: 1`.
- Provide `getClient()` → acquires the pooled connection.
- `beginTransaction(client)` → `client.query('BEGIN')`.
- `setLocalRole(client)` → `client.query('SET LOCAL ROLE amaia_sync_runtime')`.
- `commit(client)` → `client.query('COMMIT')`.
- `rollback(client)` → `client.query('ROLLBACK')`.
- All SECURITY DEFINER function invocations via `SELECT amaia_sync_finalize_source(...)` etc.
- All repository operations receive the `client` (PoolClient) as parameter.

**Transaction lifecycle** owned by domain-runner. Repositories never create or commit transactions.

**Why not supabase-js:** supabase-js uses `PostgREST` which wraps each request in its own transaction. The fenced transaction requires multiple statements (12 steps) sharing a single transaction context with explicit `BEGIN`/`COMMIT`/`ROLLBACK`. This is only possible via direct PostgreSQL connectivity.

**Service-role key** may still be used elsewhere in the Mistatas application (e.g., edge functions, dashboard). The Runtime Engine does not use supabase-js at all.
...
|---|---|---|---|
| `SUPABASE_DB_URL` | string | — | Required, PostgreSQL connection string (session pooler) |
...
| R12 No evidence before commit | pg-client.ts: tx boundary (direct pg driver) |
...
| R27 Terminal predicate | manifest-service.ts: guardedTerminalization via SQL `now()` evaluation (Section 11.1). JS clock/cached lease prohibited. |
...
| R31 Single terminal op | manifest-service.ts: SQL-evaluated predicate + complete + status in one sequence |
| R32 Fresh predicate | manifest-service.ts: `lease_expires_at > now()` evaluated by PostgreSQL, not JS |
