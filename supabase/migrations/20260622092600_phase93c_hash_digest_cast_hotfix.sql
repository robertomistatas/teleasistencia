...
-- Cast hash input to bytea explicitly for remotes where pgcrypto.digest(text, text)
-- is not available.
...
begin
  ...
  item_hash:=encode(digest(convert_to(array_to_string(v_els,v_delim),'UTF8'),'sha256'),'hex');
  return next;
end; $f$;
