-- =============================================
-- phase4_4a_call_correlation_foundation
-- Base operacional para llamadas crudas y
-- correlacion historica auditable.
-- =============================================

create type public.call_correlation_status as enum (
	'matched_single',
	'matched_multiple',
	'unmatched',
	'invalid_phone'
...
create type public.call_match_method as enum (
	'phone_exact_active_contact',
	'phone_exact_inactive_contact',
	'phone_exact_multiple_contacts',
	'no_contact_match',
	'invalid_phone'
);

create table public.raw_call_logs (
	...
	raw_beneficiary_label text,
	raw_observations text,
	raw_payload jsonb not null default '{}'::jsonb,
	created_at timestamptz not null default now(),
	created_by uuid references public.profiles (id) on delete set null,
	constraint raw_call_logs_source_not_blank check (btrim(source) <> ''),
	constraint raw_call_logs_duration_seconds_check check (duration_seconds is null or duration_seconds >= 0)
);

create unique index idx_raw_call_logs_source_external_call_id_unique
	on public.raw_call_logs (source, external_call_id)
	where external_call_id is not null;

create index idx_raw_call_logs_phone_normalized
	on public.raw_call_logs (phone_normalized);

create index idx_raw_call_logs_called_at
	on public.raw_call_logs (called_at);

create table public.call_correlations (
	id uuid primary key default gen_random_uuid(),
	raw_call_log_id uuid not null references public.raw_call_logs (id) on delete cascade,
	correlation_status public.call_correlation_status not null,
	...
	match_method public.call_match_method not null,
	confidence_score integer not null default 0,
	assignment_id_at_call_time uuid references public.beneficiary_assignments (id) on delete set null,
	responsible_user_id_at_call_time uuid references public.profiles (id) on delete set null,
	reason text not null,
	created_at timestamptz not null default now(),
	created_by uuid references public.profiles (id) on delete set null,
	updated_at timestamptz not null default now(),
	updated_by uuid references public.profiles (id) on delete set null,
	constraint call_correlations_raw_call_log_id_unique unique (raw_call_log_id),
	constraint call_correlations_confidence_score_check check (confidence_score >= 0 and confidence_score <= 100)
);

create index idx_call_correlations_beneficiary_id
	on public.call_correlations (beneficiary_id);

create index idx_call_correlations_responsible_user_id_at_call_time
	on public.call_correlations (responsible_user_id_at_call_time);

create or replace function public.sync_raw_call_log_phone_normalized()
returns trigger
language plpgsql
set search_path = public
as $$
begin
	new.phone_normalized := public.normalize_chilean_phone(new.raw_phone);
	return new;
end;
$$;

create trigger raw_call_logs_set_phone_normalized
	before insert or update of raw_phone
	on public.raw_call_logs
	for each row
	execute function public.sync_raw_call_log_phone_normalized();

create trigger call_correlations_set_updated_at
	before update on public.call_correlations
	for each row
	execute function public.set_updated_at();

alter table public.raw_call_logs enable row level security;
alter table public.call_correlations enable row level security;

create policy "raw_call_logs_select_admin_super_admin"
	on public.raw_call_logs
	for select
	to authenticated
	using (
...
create policy "call_correlations_select_admin_super_admin"
	on public.call_correlations
	for select
	to authenticated
	using (
		public.get_user_role((select auth.uid())) in ('admin', 'super_admin')
...
comment on table public.raw_call_logs
	is 'Registro crudo e inmutable a nivel semantico de llamadas externas antes de correlacion operacional.';

comment on table public.call_correlations
	is 'Resultado auditable e idempotente del intento de correlacion operacional de una llamada cruda.';

comment on function public.sync_raw_call_log_phone_normalized()
	is 'Sincroniza phone_normalized en raw_call_logs usando public.normalize_chilean_phone.';
